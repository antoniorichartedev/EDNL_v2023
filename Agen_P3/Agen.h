...
template <typename T>
int Agen<T>::calcularaltura(nodo n) const {
    if (n == Agen<T>::NODO_NULO){
        return 0;
    }
    else{
        return 1 + std::max(calcularaltura(hijoIzqdo(n)), calcularaltura(hermDrcho(n)));
    }
}

template <typename T>
int Agen<T>::altura(nodo n) const {
    if (n == Agen<T>::NODO_NULO){
        return 0;
    }
    else{
        return std::max(calcularaltura(hijoIzqdo(n)), calcularaltura(hermDrcho(n)));
    }
}
